...
def initialize_db():
    ...
    cur.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS recetas (
            ...
            porciones_rinde INTEGER NOT NULL DEFAULT 1,
            notas           TEXT,
            created_at      TEXT    DEFAULT (datetime('now','localtime')),
            updated_at      TEXT    DEFAULT (datetime('now','localtime'))
        );
        ...
        CREATE TABLE IF NOT EXISTS receta_ingredientes (
            ...
            alimento_id     INTEGER,
            nombre_alimento TEXT    NOT NULL,
            gramos          REAL    NOT NULL DEFAULT 100,
            medida_casera   TEXT,
            ...
            FOREIGN KEY (alimento_id) REFERENCES alimentos_db(id) ON DELETE SET NULL
        );
        ...
        CREATE TABLE IF NOT EXISTS receta_equivalencias (
            ...
            receta_id   INTEGER NOT NULL,
            grupo       TEXT    NOT NULL,
            porciones   REAL    NOT NULL DEFAULT 0,
            FOREIGN KEY (receta_id) REFERENCES recetas(id) ON DELETE CASCADE
        );
    """)
...
def insert_receta(data: dict) -> int:
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO recetas (nombre, descripcion, categoria, porciones_rinde, notas)
        VALUES (:nombre, :descripcion, :categoria, :porciones_rinde, :notas)
    """, data)
    conn.commit()
    rid = cur.lastrowid
    conn.close()
...
def update_receta(rid: int, data: dict):
    data["id"] = rid
    data["updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = get_connection()
    conn.execute("""
        ...
        SET nombre=:nombre, descripcion=:descripcion, categoria=:categoria,
            porciones_rinde=:porciones_rinde, notas=:notas, updated_at=:updated_at
        WHERE id=:id
    """, data)
    conn.commit()
    conn.close()
...
def get_all_recetas() -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM recetas ORDER BY nombre COLLATE NOCASE"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_receta(rid: int) -> Optional[dict]:
    conn = get_connection()
    row = conn.execute("SELECT * FROM recetas WHERE id=?", (rid,)).fetchone()
    conn.close()
    return dict(row) if row else None


def search_recetas(query: str) -> list:
    conn = get_connection()
    like = f"%{query}%"
    rows = conn.execute(
        """SELECT * FROM recetas
...
def get_receta_ingredientes(rid: int) -> list:
    conn = get_connection()
    rows = conn.execute(
...
def insert_receta_ingrediente(data: dict) -> int:
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO receta_ingredientes
            (receta_id, alimento_id, nombre_alimento, gramos, medida_casera,
             calorias, proteinas_g, carbohidratos_g, grasas_g, fibra_g)
        VALUES (:receta_id, :alimento_id, :nombre_alimento, :gramos, :medida_casera,
                :calorias, :proteinas_g, :carbohidratos_g, :grasas_g, :fibra_g)
    """, data)
    conn.commit()
    iid = cur.lastrowid
...
def delete_receta_ingrediente(iid: int):
    conn = get_connection()
    conn.execute("DELETE FROM receta_ingredientes WHERE id=?", (iid,))
    conn.commit()
    conn.close()


def get_receta_equivalencias(rid: int) -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM receta_equivalencias WHERE receta_id=? ORDER BY id",
        (rid,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def save_receta_equivalencias(rid: int, equivalencias: dict):
    """Replaces all equivalencias for a recipe with {grupo: porciones} dict."""
    conn = get_connection()
    conn.execute("DELETE FROM receta_equivalencias WHERE receta_id=?", (rid,))
    for grupo, porciones in equivalencias.items():
        if porciones > 0:
            conn.execute(
                "INSERT INTO receta_equivalencias (receta_id, grupo, porciones) VALUES (?,?,?)",
                (rid, grupo, porciones)
            )
    conn.commit()
    conn.close()
